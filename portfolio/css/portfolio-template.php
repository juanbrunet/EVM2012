<?php
	...
	$total_pages = ceil((int)($post_count->publish) / 40);
	...
	<div class="portfolio">
		<?php foreach($portfolio->posts as $key => $portfolio_item): ?>
			<?php 
				...
				$last = $portfolio_type == 'gallery' && ($key + 1) % 3 == 0 ? 'last' : $last ;
			...
			$consulta = "SELECT * FROM wp_term_relationships WHERE object_id = " . $portfolio_item->ID;
			   ...
			   if ($_GET["page_id"] == "160")
			      ...
			      $consulta = "SELECT * FROM wp_term_relationships WHERE term_taxonomy_id in (100, 101, 102, 4, 99, 133) AND object_id = " . $portfolio_item->ID;
                        }
			...
			?>
			<div id="data0-<?php echo $portfolio_item->ID ?>" class="portfolio-item-wrapper <?php echo $last ?>">
